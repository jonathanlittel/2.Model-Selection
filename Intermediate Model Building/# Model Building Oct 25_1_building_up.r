...
modelColumns0 <- c("WriteoffsDummy", "Sales_log")
...
modelCols2 <- c(modelColumns0,
                "max_pct_per_buyer", "multiple_segments")
...
modelCols5 <- c(modelColumns0,
                "max_pct_per_buyer", "multiple_segments",
                "EBITDA.Total.Debt.Service", "Cash.Conversion.Cycle.")
...
modelCols8 <- c(modelColumns0,
                "max_pct_per_buyer", "multiple_segments",
                "EBITDA.Total.Debt.Service", "Cash.Conversion.Cycle.", "Total.Asset.Turnover",
                "Depth.of.Management", "Product.Recognition")
...
#####
modelCols10 <- c(modelColumns0,
                "max_pct_per_buyer", "multiple_segments",
                "EBITDA.Total.Debt.Service", "Cash.Conversion.Cycle.", "Total.Asset.Turnover",
                "Depth.of.Management", "Product.Recognition", "Financial.Flexibility", 
                "Financial.Strat.Quality")

df.train.model <- df.train[,names(df.train) %in% modelCols10]
glm10 <- glm(WriteoffsDummy ~ ., data=df.train.model, family="binomial", na.action=na.exclude)
summary(glm10)
...
df.train.model <- df.train.model[complete.cases(df.train.model),]
glm10step <- stepAIC(glm10, direction=c("both"))
### Previous
...
glm9 <- glm(WriteoffsDummy ~ ., data=df.train.model, family="binomial", na.action=na.exclude)
summary(glm9)
...
compTable <- stargazer(glm0, glm1, glm2, glm3, glm4, glm5, glm6, glm7, glm8,
                       glm9, glm10, type = "html",
                       ci = TRUE)

write(compTable, file = "step_comparison_rr.html", append = TRUE)

rocs <- c(roc0$auc,roc1$auc,roc2$auc,
            roc3$auc,roc4$auc,roc5$auc,roc6$auc,
            roc7$auc,roc8$auc,roc9$auc,roc10$auc)
write(rocs, file = "step_comparison_rr.html", append = TRUE)
