...
df.rap$past_arrears <- ifelse(df.rap$Max_Client_Arrears_Before_Close_1to15days>=4,1,0)

# df.rap$past_arrears <- as.factor(df.rap$Max_RiskCategory_Before_Close)

df.rap$active_loan <- ifelse(df.rap$balance_0615>0,1,0)
...
modelCols7 <- c("WriteoffsDummy", "Sales_log", "WorkingCapital_log",
                "Max_Client_ArrearsandRiskCategory_Before_Close",
                "DtoE_adding_RC_Loan", "margin_sd","Depth.of.Management",
                "isCoffee")
...
write(compTable, file = "comparison_finalz.html", append = TRUE)

rocs <- c(roc0$auc,roc1$auc,roc2$auc,
          roc3$auc, roc4$auc, roc5$auc, roc6$auc, roc7$auc,
          roc8$auc)
write("ROCs:", file = "comparison_finalz.html", append = TRUE)
write(rocs, file = "comparison_finalz.html", append = TRUE)

finalPD <- stargazer(glm8, type = "html",
                     ci = TRUE)
write(finalPD, file = "pd_modelz.html", append = TRUE)

write.csv(glm4$coefficients, file="pd_model_finalz.csv")

varImp(glm8) # from caret package 

quantile(df.rap$predicted_wo, probs = seq(0, 1, 0.05), na.rm=TRUE)